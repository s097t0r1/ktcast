...
import me.s097t0r1.shelf.serializer.MoshiSerializer
...
open class Shelf(var storage : Storage, var serializer: Serializer, var clock : Clock = Clock()) {
    ...
    companion object {
        fun create(context: Context) = Shelf(SharedPreferencesStorage(context), MoshiSerializer())
    }
    ...
    class Item(val key: String, val shelf: Shelf) {
        ...
        fun age(): Long? = try {
            shelf.storage.timestamp(key)?.let { shelf.clock.now() - it }
        } catch (e: Throwable) {
            null
        }
